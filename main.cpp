...
private:
    ...
    void drawHeader()
    {
        ...
        cout << "Made by: 林君諺 BeluBruh123 && 游昊軒 PaceyNotFound\n";
        cout << "BeluBruh123's Github: github.com/belubruh123\n";
        cout << "BeluBruh123's YouTube: youtube.com/@mgbgame\n";
        cout << "BeluBruh123's Discord: mgb_dev\n";
...
public:
    ...
    void show()
    {
        ...
        while (running)
        {
            ...
            cout << endl
                 << BORDER_RED << "=========================================================" << RESET << endl;

            int ch = getch();
            if (ch == 0 || ch == 224)
            { // Windows arrow keys
                ch = getch();
                if (ch == 72 && selectedIndex > 0)
                    selectedIndex--;
                if (ch == 80 && selectedIndex < options.size() - 1)
                    selectedIndex++;
            }
            else if (ch == 27)
            { // Linux/macOS arrow keys
                ...
                {
                    ch = getch();
                    if (ch == 65 && selectedIndex > 0)
                        selectedIndex--;
                    if (ch == 66 && selectedIndex < options.size() - 1)
                        selectedIndex++;
                }
            }
            else if (ch == 10 || ch == 13)
